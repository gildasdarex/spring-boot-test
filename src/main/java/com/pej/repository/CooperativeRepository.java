...
public interface CooperativeRepository extends CrudRepository<Cooperative, Integer> {
	...
	@Query("select co from Cooperative co where co.idgroupe  not in (select fc.cooperative.idgroupe from Formationcooperative fc where fc.cooperative.idgroupe=?1)" )
	Iterable<Cooperative> getNotInFormationCooperative(Integer id);
	
	@Query("select co from Cooperative co where co.idgroupe  in (select fc.cooperative.idgroupe from Formationcooperative fc where fc.cooperative.idgroupe=?1)" )
